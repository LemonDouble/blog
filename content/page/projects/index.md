...
Babylon.js + babylon-mmd로 PMX 모델을 렌더링하고, 표정 모프 타겟 16종, VMD 모션 14종, Bullet WASM으로 머리카락/옷감 물리 시뮬레이션까지 지원합니다.
LLM 기반 대화 기능은 연동 예정!
...
HomeAssistant에서 ADB로 TV에 띄우기 위해 만든 날씨 앱입니다. 기상청 예보 강수 유형에 따라 비/눈 같은 날씨 애니메이션을 보여줍니다. 개인용이라 별거 없긴 한데, 매일 아침 날씨 브리핑하면 자전거 타고 출근할지 말지 정할 수 있어서 편합니다.
...
맛집 평가 시스템입니다. 룰렛 뽑기, 별점 평가, Google Maps URL 자동 복사, QR 코드 생성까지 지원합니다. 금요일마다 친구들과 맛집탐방 가는데, 이왕 먹는 김에 별점도 매기면 좋겠다 싶어서.. 익명 투표 시스템으로 만들었습니다.
...
`~/.claude/projects/`의 JSONL 파일을 파싱해서 Claude Code 사용량(비용, 토큰, 세션, 패턴)을 시각화하는 웹 대시보드입니다. Docker Compose로 띄우면 끝. 내가 얼마나 Claude를 혹사시키고 있는지 한눈에 볼 수 있습니다(...).

### [Style-Bert-VITS2](https://github.com/LemonDouble/Style-Bert-VITS2) (Fork)
> Style-Bert-VITS2 기반 일본어 TTS 라이브러리

- **언어**: <i class="devicon-python-plain colored"></i> Python
- **상태**: 개발중
- **원본**: [litagin02/Style-Bert-VITS2](https://github.com/litagin02/Style-Bert-VITS2)

원본 프로젝트를 `pip install style-bert-vits2`로 바로 설치할 수 있게 라이브러리로 리패키징하고 있습니다.
ONNX 런타임으로 CPU에서도 실시간 추론이 가능하고, 패키지 다운받고 3줄이면 추론 가능하게 만들 예정입니다.

### [kepco_powerplanner_ha_addon](https://github.com/LemonDouble/kepco_powerplanner_ha_addon) (Fork)
...
그래서 MQTT Discovery 방식으로 전환해서 `unique_id`를 부여하고, HA UI에서 센서 이름도 바꾸고 관리할 수 있게 고쳤습니다.
Supervisor API를 통한 MQTT 브로커 자동 감지도 추가해서 설정도 간편하게 만들었습니다.

### [claude-slack-bridge](https://github.com/LemonDouble/claude-slack-bridge) (Fork)
> Claude Code ↔ Slack 양방향 브릿지 MCP 서버

- **언어**: <i class="devicon-python-plain colored"></i> Python
- **상태**: 완료
- **원본**: [tomeraitz/claude-slack-bridge](https://github.com/tomeraitz/claude-slack-bridge)

Claude Code한테 일 시켜놓고 자리 비울 때 쓰려고 만든 MCP 서버입니다. 원본에서 꽤 많이 확장했는데, 주요 기능은 이렇습니다:

- **MCP 도구 4종**: `ask_on_slack`(질문 보내고 답변 대기), `notify_on_slack`(알림만 보내기), `upload_to_slack`(파일 업로드), `download_slack_file`(Slack 첨부파일 다운로드)
- **Slack → Claude**: Slack에서 봇 멘션하면 프로젝트 목록이 뜨고, 선택하면 해당 프로젝트에서 Claude 세션이 시작됩니다. 새 프로젝트 생성도 Slack에서 바로 가능.
...
한국어/영어 등을 입력하면 자동 번역 후 일본어 음성으로 출력해주고, API 서버에서 스피커 목록을 가져와서 스피커별 TTS 엔티티를 자동 생성합니다.

### [kanban](https://github.com/Lemon-Obsidian/kanban)
> 옵시디언 칸반 플러그인
...
카드가 `.md` 파일이라 LLM 먹여서 쓸 수도 있고, 아카이브 처리하면 개인 업무노트 용도로도 좋습니다.
옵시디언은 풀 로컬이니까요.

### [write-helper](https://github.com/Lemon-Obsidian/write-helper)
> LLM 기반 글쓰기 보조 옵시디언 플러그인

- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript
- **상태**: 개발중

노트 저장 시 LLM이 자동으로 태그를 추천하고, YAML 정의 폼 템플릿으로 필드별 LLM 프롬프트 설정이 가능한 플러그인입니다. 뭔가 글 쓰려고 만들었는데 바빠서 안 쓰게 되긴 하네요(...).

### [image-manager](https://github.com/Lemon-Obsidian/image-manager)
...
### [git-sync](https://github.com/Lemon-Obsidian/git-sync)
> 옵시디언 Git 동기화 플러그인

- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript
- **상태**: 완료

기존에 git 플러그인이 있긴 한데.. 맨날 볼트 나가기 전에 동기화를 까먹어서 만들었습니다. X 버튼 누르면 commit→push 자동으로 해주니까 까먹는 일이 훨씬 줄었습니다.

### [lemon-pyopenjtalk-prebuilt](https://github.com/LemonDoubleHQ/lemon-pyopenjtalk-prebuilt) (Fork)
...
### [skopeo-setup-action](https://github.com/LemonDouble/skopeo-setup-action)
> GitHub Actions에서 skopeo 셋업

- **상태**: 완료

GitHub Actions 러너에 [Skopeo](https://github.com/containers/skopeo)를 설치하는 액션입니다. 셀프호스트 OCI 호환 레지스트리에 `docker buildx`만으로는 레이어 업데이트를 못 해서.. 필요한 액션인데 마땅한 게 없길래 직접 만들어 쓰기로 했습니다.

### [misskey](https://github.com/LemonDouble/misskey) (Fork)
> 셀프호스팅 마이크로블로깅 플랫폼

- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript
- **상태**: 운영중
- **원본**: [misskey-dev/misskey](https://github.com/misskey-dev/misskey)
...
### [fastapi-tdd](https://github.com/LemonDouble/fastapi-tdd)
> FastAPI TDD 강의 실습

- **언어**: <i class="devicon-python-plain colored"></i> Python
- **상태**: 완료

취업하고 나서 잠깐 여유가 생겼을 때 Udemy의 "Try FastAPI - Test Driven Development 2024" 강의를 따라하며 TDD를 공부해본 레포지토리입니다.
...
- **언어**: <i class="devicon-yaml-plain colored"></i> YAML
- **상태**: 운영중
...
- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript, <i class="devicon-python-plain colored"></i> Python
- **상태**: archived
- **레포지토리**: frontend / ml-server 내 lemon-waifu 모듈 (모두 비공개)

애니메이션 캐릭터 이미지 한 장만 있으면 브라우저에서 그 캐릭터와 대화할 수 있는 프로젝트입니다.
[talking-head-anime-3-demo](https://github.com/pkhungurn/talking-head-anime-3-demo)를 ML 서버에 올려서 상반신 애니메이션을 생성하고, MediaPipe로 얼굴 랜드마크를 추출해서 캐릭터에 매핑했습니다.
...
### [lemon-main-page](https://github.com/LemonDouble/lemon-main-page)
> 개인 자기소개 페이지

- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript
- **상태**: 운영중

[lemondouble.com](https://lemondouble.com)에서 운영중인 개인 포트폴리오/자기소개 페이지입니다.

### [blog](https://github.com/LemonDouble/blog)
> 개인 블로그

- **언어**: <i class="devicon-hugo-plain colored"></i> Hugo (<i class="devicon-css3-plain colored"></i> CSS)
- **상태**: 운영중

Hugo + GitHub Pages 기반 개인 블로그. 지금 보고 있는 바로 이 블로그입니다!

### [chatgpt-serverless-discord-bot](https://github.com/LemonDouble/chatgpt-serverless-discord-bot)
> ChatGPT API + Lambda Serverless Discord Bot 템플릿

- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript
- **상태**: archived

ChatGPT API와 AWS Lambda를 이용해서 Serverless Discord Bot을 만들 수 있는 템플릿입니다. 매번 봇 하나 돌리자고 서버 띄우면 비싸니까요~

### [chatgpt-serverless-slack-bot](https://github.com/LemonDouble/chatgpt-serverless-slack-bot)
> ChatGPT API + Lambda Serverless Slack Bot 템플릿

- **언어**: <i class="devicon-typescript-plain colored"></i> TypeScript
- **상태**: archived

마찬가지로 ChatGPT API + AWS Lambda 기반 Serverless Slack Bot 템플릿입니다.

### [lemon-chatgpt-translator](https://github.com/LemonDouble/lemon-chatgpt-translator)
...
### [twitter-remove-doge](https://github.com/LemonDouble/twitter-remove-doge)
...
- **언어**: <i class="devicon-java-plain colored"></i> Java, <i class="devicon-typescript-plain colored"></i> TypeScript, <i class="devicon-python-plain colored"></i> Python
- **상태**: archived
- **레포지토리**: backend / frontend / bert-encoder / lambda-functions / bert-answer-api (모두 비공개)
- **회고**: [Lemon Toolbox 첫 런칭 회고](/p/lemon-toolbox/)
...
- **언어**: <i class="devicon-javascript-plain colored"></i> JavaScript
- **상태**: 드랍
...
### [Beacon_based_Automatic_visitor_check_program](https://github.com/LemonDouble/Beacon_based_Automatic_visitor_check_program_GBPL) (GBPL)
> 블루투스 비콘 기반 방문일지 자동화

- **언어**: <i class="devicon-java-plain colored"></i> Java (<i class="devicon-android-plain colored"></i> Android)
- **상태**: archived
...
### [Audio_processing_practice](https://github.com/LemonDouble/Audio_processing_practice)
> 음성 처리 스크립트 모음

- **언어**: <i class="devicon-python-plain colored"></i> Python (<i class="devicon-jupyter-plain colored"></i> Jupyter Notebook)
- **상태**: archived
...
### [Socket_Order_Program](https://github.com/LemonDouble/Socket_Order_Program)
> C++ 소켓/스레드 기반 음식 주문 서버/클라이언트

- **언어**: <i class="devicon-cplusplus-plain colored"></i> C++
- **상태**: archived

대학 네트워크 소켓 프로그래밍 수업 과제인데, 발표 하루 전에 밤새서 겨우겨우 만든 기억이 납니다(...). C++로 로우레벨 소켓 통신을 처음 해보고, 패킷 크기도 직접 정해서 커스텀 통신을 해봤는데 나름 재밌었습니다.

### [C_minus_compiler](https://github.com/LemonDouble/C_minus_compiler)
> C minus 컴파일러 구현

- **언어**: <i class="devicon-cplusplus-plain colored"></i> C++
- **상태**: archived

대학 컴파일러 수업 과제입니다. Scanner랑 Parser까지 구현했는데, 더 하고 싶었지만 학부 한 학기는 짧아서 여기까지만.. 언젠가 기회가 된다면 한번 내 언어를 만들어보고 싶네요. 재밌었거든요.

### [Simple_IR](https://github.com/LemonDouble/Simple_IR)
> TF-IDF 기반 간단한 정보검색 엔진

- **언어**: <i class="devicon-python-plain colored"></i> Python
- **상태**: archived

대학 정보검색(검색엔진) 수업 과제입니다. 문서를 읽어서 TF-IDF weight dictionary로 만들고, 쿼리와의 유사도를 측정해서 상위 10개 결과를 출력합니다.

### [react-test-practice](https://github.com/LemonDouble/react-test-practice)
> 리액트 테스트 라이브러리 연습

- **언어**: <i class="devicon-javascript-plain colored"></i> JavaScript
- **상태**: archived

리액트 테스팅을 처음 공부해봤습니다. 당시에 취업한다고 이것저것 참 많이도 공부했네요..

### [github-actions-practice](https://github.com/LemonDouble/github-actions-practice)
> GitHub Actions 실습

- **상태**: archived

GitHub Actions를 처음 배워봤습니다.

### [Spring_sample_Simple_Forum](https://github.com/LemonDouble/Spring_sample_Simple_Forum)
> 스터디용 게시판 만들기

- **언어**: <i class="devicon-java-plain colored"></i> Java
- **상태**: archived

Spring을 처음 써본 프로젝트입니다. 간단한 시스템부터 단계별로 요구사항을 추가하면서, 객체 지향 설계랑 테스트가 왜 필요한지 고민해봤습니다.

### [Skyos](https://github.com/LemonDouble/Skyos)
> C++로 나만의 OS 만들기

- **언어**: <i class="devicon-cplusplus-plain colored"></i> C++
- **상태**: 드랍

OS를 직접 만들어보려 했는데.. 당시에는 너무 어려운 과제였습니다.

### [python_architecture](https://github.com/LemonDouble/python_architecture) (Fork)
> Architecture Patterns with Python 책 실습

- **언어**: <i class="devicon-python-plain colored"></i> Python
- **원본**: [cosmicpython/code](https://github.com/cosmicpython/code)
...
### [Omok](https://github.com/LemonDouble/Omok) / [Snake_Game](https://github.com/LemonDouble/Snake_Game) / [Simple_SQL](https://github.com/LemonDouble/Simple_SQL)
> 대학 1학년 시절의 프로젝트들

- **언어**: <i class="devicon-cplusplus-plain colored"></i> C++ / <i class="devicon-c-plain colored"></i> C
- **상태**: 완료

대학교 1학년 때 C/C++ 배우면서 만들어본 것들입니다. 오목 게임, System Call 기반 스네이크 게임, 그리고 MySQL의 기본 기능을 흉내낸 간단한 SQL 구현. 1학년 때 한 거니까 귀엽게 봐주세요(...). 당시엔 진짜 어려웠답니다..
